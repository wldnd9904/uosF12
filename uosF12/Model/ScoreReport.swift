import Foundation
let validGradeList:[String] = ["A+","A0","B+","B0","C+","C0","D+","D0","F"]
//성적표
public struct ScoreReport {
    ...
    public func sum(div:String) -> Int {
        if Subjects.filter({div=="" || $0.subjectDiv.rawValue.contains(div)}).isEmpty { return 0 }
        else {
            return Subjects.reduce(0){
                $0 + (div=="" || $1.subjectDiv.rawValue.contains(div) ? $1.pnt : 0)
            }
        }
    }
    
    public func nonPassSum(div:String) -> Int {
        if Subjects.filter({(div=="" || $0.subjectDiv.rawValue.contains(div)) && validGradeList.contains($0.gradeStr)}).isEmpty { return 1 }
        else {
            return Subjects.reduce(0){
                $0 + (((div=="" || $1.subjectDiv.rawValue.contains(div)) && validGradeList.contains($1.gradeStr)) ? $1.pnt : 0)
            }
        }
    }
    
    public func avg(div:String) -> Float {
        if Subjects.filter({div=="" || $0.subjectDiv.rawValue.contains(div)}).isEmpty { return 0.0 }
        else {
            return Subjects.reduce(0.0){
                $0 + ((div=="" || $1.subjectDiv.rawValue.contains(div)) && validGradeList.contains($1.gradeStr) ? (Float($1.pnt) * $1.grade) : 0)
            } / Float(nonPassSum(div:div))
        }
    }
